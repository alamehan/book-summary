...
Template Vue mendukung JavaScript Expressions seperti ```{{ `Diskon: ${total * 10%}` }}```, ```{{ ok ? 'YES' : 'NO' }}```, ```{{ message.split('').reverse().join('') }}```. Atau jika dalam bentuk atribut HTML maka penulisannya sebagai berikut ```<h1 :id="`product-${index}`"></h1>```.
